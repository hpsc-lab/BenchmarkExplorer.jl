...
function get_package_version(package_name::String)
    try
        deps = Pkg.dependencies()
        pkg_uuid = findfirst(d -> d.name == package_name, deps)
        return isnothing(pkg_uuid) ? "unknown" : string(deps[pkg_uuid].version)
    catch
        return "unknown"
    end
end
